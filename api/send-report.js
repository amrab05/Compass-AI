...
export default async function handler(request, response) {
    ...
    try {
        ...
        const htmlBody = `
            <div style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: auto; border: 1px solid #e2e8f0; border-radius: 8px; overflow: hidden;">
                <div style="background-color: #4c3c7a; color: white; padding: 20px;">
                    <h2 style="margin: 0;">Your Compass-AI Analysis</h2>
                </div>
                <div style="padding: 20px;">
                    <p>${greeting}</p>
                    <p>Thank you for using Compass-AI! Here is the AI-powered de-escalation report you requested:</p>
        ...
        await postmark.sendEmail({ "From": fromAddress, "To": leadEmail, "Subject": "Your Compass-AI Analysis Report", "HtmlBody": htmlBody });
        await postmark.sendEmail({ "From": fromAddress, "To": fromAddress, "Subject": "New Lead from Compass-AI!", "HtmlBody": `New lead captured: <strong>${leadIdentifier}</strong>` });
